...
export default function AsXML() {
	...
	async function viewXML(limit) {
		setIsLoading(true);
		try {
			const result = await invoke("export_db_view", { format: "xml", limit });
			setXML(result);
		} catch (err) {
						...
						<Flex flexDir='column'>
							...
							<Box alignSelf='end'>
								<Tooltip label={collapse ? "Expand" : "Collapse"}>
									<IconButton icon={collapse ? <BsArrowsExpand size='1.4em' /> : <BsArrowsCollapse size='1.4em' />} onClick={() => setCollapse(!collapse)} variant='ghost' colorScheme='green' />
								</Tooltip>
							</Box>
							{isLoading ? (
