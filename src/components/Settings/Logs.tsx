import ViewportListDefault from "react-viewport-list";
import { useState, useEffect, useRef } from "react";
import { type LogEntry } from "@Types";

const ViewportList = ViewportListDefault as unknown as React.FC<{
	ref?: React.Ref<unknown>;
	viewportRef?: React.RefObject<HTMLElement | null>;
	itemMinSize?: number;
	items: LogEntry[];
	children: (item: LogEntry, index: number) => React.ReactNode;
}>;
...
function LogAlert({ status, date, content }: LogEntry) {
	const availableTypes = ["warning", "error", "info"];

	return (
		<Alert
			status={
				(availableTypes.includes(status) ? status : "warning") as
					| "warning"
					| "error"
					| "info"
			}>
...
const LogRows = ({ data }: { data: LogEntry[] }) => {
