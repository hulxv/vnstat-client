...
function Info() {
	...
	const [information, setInfos] = useState<AppInfo[]>([]);
	const { config } = useConfig();

	useEffect(() => {
		invoke<AppInfo[]>("get_infos")
	...
	return (
		<Stack spacing={4} alignSelf="center" align="center">
			...
			<Button
				...
				onClick={() =>
					invoke("open_url", {
						url: "https://github.com/Hulxv/vnstat-client/releases",
					}).catch(console.error)
				}>
				Check for updates
			</Button>
			...
			{information.length > 0 && (
				<Tooltip
					hasArrow
					placement="top"
					label={`Show ${showMoreInfo ? "less" : "more"} information about vnStat`}>
					<IconButton
						aria-label={`Show ${showMoreInfo ? "less" : "more"} information`}
						variant="ghost"
